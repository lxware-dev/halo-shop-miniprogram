...
当进行本地二次开发且不想经过后台服务时，可以启用 `Mock` 模式。或者当目前不具有小程序 AppId 及 AppSecret 时，可作为临时模式启动。
...
> [!NOTE]
> 开发调试时，可在微信开发者工具中临时勾选「不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书」，以便在未配置合法域名的情况下进行本地联调。**真机预览与正式发布前必须完成合法域名配置**，详见 [4.prepare-go-live.md](./4.prepare-go-live.md)。

## 下一步

配置完上面的步骤后，可能会发现小程序卡在首页请求，这是因为还没有进行 Halo 商业版后台的联调。

在小程序启动后，还需要在 Halo 后台完成小程序 AppId 设置、小程序 Banner、快捷入口、小程序支付方式等数据录入，各核心流程才能正常跑通。

详见 [3.prepare-backend.md](./3.prepare-backend.md)。
