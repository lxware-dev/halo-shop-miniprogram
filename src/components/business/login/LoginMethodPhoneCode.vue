...
async function sendCode() {
  if (!phone.value.trim()) {
    uni.showToast({ title: t('login.enterPhone'), icon: 'none' });
    return;
  }
  if (countdown.value > 0) {
    return;
  ...
  try {
    await sendRequest(authApi.sendCode({ target: phone.value.trim(), type: 'sms' }));
    startCountdown();
...
<template>
  <view class="flex flex-col gap-2.5">
    ...
    >
      <TIcon name="mobile" v-bind="{ size: '36rpx', color: ICON_COLOR.muted }" />
      <input
        v-model="phone"
